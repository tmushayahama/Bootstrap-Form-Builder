define([
	...
	return Backbone.View.extend({
		...
		, initialize: function() {
			...
			this.collection.on("remove", this.removeComponent, this);
		...
		, wrapRooms: function() {
			...
			var i = 0;
			while (i < $set.length) {
				if ($($set[i]).hasClass("fb-house-full")) {
					$set.slice(i, i + 1).wrapAll('<div class="row fb-house-full component-row"/>');
					i += 1;
				} else if ($($set[i]).hasClass("col-lg-4")) {
					$set.slice(i, i + 3).wrapAll('<div class="row fb-house-full component-row"/>');
					i += 3;
